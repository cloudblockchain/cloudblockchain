...
static CTransactionRef SendMoneyAndSignCopyRight(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, const CCoinControl& coin_control, mapValue_t mapValue, std::string fromAccount, CopyRight &right)
{
    ...
    vecSend.push_back(recipient);
    CTransactionRef tx;
    if (!pwallet->CreateCopyRightTransaction(vecSend, tx, reservekey, nFeeRequired, nChangePosRet, strError, coin_control, right)) {
        if (!fSubtractFeeFromAmount && nValue + nFeeRequired > curBalance)
            strError = strprintf("Error: This transaction requires a transaction fee of at least %s", FormatMoney(nFeeRequired));
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
...
UniValue uploadfile(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            "uploadfile \"filename\"\n"
            "\nupload a file to masternodes.\n"
            "\nArguments:\n"
            "1. \"filename\"            (string, required) The absoluted path of the file to be uploaded.\n"
            "\nExamples:\n"
            + HelpExampleRpc("uploadfile",  "\"data/src/sun.jpg\"")
        );
	...
	vInv.push_back(CInv(MSG_FILE, filename));
	
	CNode* pnode = g_connman->GetMasterNode();
	if(pnode == nullptr)
	{
		LogPrintf("No Master Nodes, File (%s) can not be uploaded\n",vInv[0].fileName);
	}
	else
	{
		LogPrint(BCLog::NET, "PushMessage:File (%s) will be uploaded to peer=%d\n",vInv[0].fileName, pnode->addr.ToStringIP());
...
UniValue signcopyright(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 4)
        throw std::runtime_error(
            "signcopyright \"toaddress\" amount \"name\" \"url\" \"address\" \"status\"\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"name\"            (string, required) the name of the digital copyright.\n"
            "2. \"url\"             (string, required) the download url of the digital copyright file.\n"
            "3. \"toaddress\"         (string, required) the cloudchain address of the digital copyright owner.\n"
			"4. \"value\"           (string, required) the value of the digital copyright.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signcopyright", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"sunshine\", \"http:\\192.168.1.128:7200\\copyrights\\sunshine.jpg\", \"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 1")
        );
    ...
    mapValue_t mapValue;
	CopyRight right;
	right.name = request.params[0].get_str();
	right.url = request.params[1].get_str();
	right.toaddress = request.params[2].get_str();
	right.value = atoi(request.params[3].get_str().c_str());
	right.status = 1;
	right.time = GetTime();
	right.id = right.ComputeHash();
...
UniValue signcopyrighttransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "3. \"url\"             (string, required) the download url of the digital copyright file.\n"
			"4. \"fromaddress\"         (string, required) the cloudchain address of the digital copyright seller.\n"
            "5. \"toaddress\"         (string, required) the cloudchain address of the digital copyright buyer.\n"
			"6. \"value\"           (string, required) the value of the digital copyright.\n"
            "\nResult:\n"
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleRpc("signcopyright", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"sunshine\", \"http:\\192.168.1.128:7200\\copyrights\\sunshine.jpg\", \"1PSSGeFHDnKNxiEyFrD1wcEaHr9hrQDDWc\", 1")
	...
	right.url = request.params[2].get_str();
	right.fromaddress = request.params[3].get_str();
	right.toaddress = request.params[4].get_str();
	right.value = atoi(request.params[5].get_str().c_str());
	right.status = 2;
	right.time = GetTime();
...
{ //  category              name                                actor (function)                argNames
	...
	{ "wallet",             "signcopyright",                    &signcopyright,                 {"name","url","address","value"} },
	{ "wallet",             "signcopyrighttransaction",         &signcopyrighttransaction,      {"id","name","url","fromaddress","toaddress","value"} },
	{ "wallet",             "uploadfile",                       &uploadfile,                    {"filename"} },
